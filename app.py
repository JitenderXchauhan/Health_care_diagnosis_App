...
model = st.sidebar.selectbox("Choose a model to make predictions", 
                             ["Heart Disease", "Parkinson's Disease", "Cancer", "Diabetes"])
...
# Function to get inputs and make predictions for Cancer
def cancer_inputs():
    st.subheader("Lungs Cancer Model Inputs")
    GENDER = st.selectbox("Gender (M-1,F-0)",options=[0,1])
    AGE = st.number_input("Age", min_value=1.0, max_value=110.0)
    SMOKING = st.selectbox("Smoking (Y-1,N-0)", options=[0,1])
    YELLOW_FINGERS = st.selectbox("Yellow fingers (Y-1,N-0)", options=[0,1])
    ANXIETY = st.selectbox("Anxiety (Y-1,N-0)", options=[0,1])
    PEER_PRESSURE = st.selectbox("Peer pressure", options=[0,1])
    CHRONIC_DISEASE = st.selectbox("Chronic Disease (Y-1,N-0)", options=[0,1])
    FATIGUE = st.selectbox("Fatiggue (Y-1,N-0)", options=[0,1])
    ALLERGY = st.selectbox("Allergy (Y-1,N-0)", options=[0,1])
    WHEEZING = st.selectbox("Wheezing (Y-1,N-0)", options=[0,1])
    ALCOHOL_CONSUMPTION = st.selectbox("Alcohol Consumption (Y-1,N-0)", options=[0,1])
    COUGHING = st.selectbox("Coughing (Y-1,N-0)", options=[0,1])
    SHORTNESS_OF_BREATH = st.selectbox("Shorteness of Breath (Y-1,N-0)", options=[0,1])
    SWALLOWING_DIFFICULTY = st.selectbox("Swallowing Difficulty (Y-1,N-0)", options=[0,1])
    CHEST_PAIN = st.selectbox("Chest Pain (Y-1,N-0)", options=[0,1])
    
    
    if st.button("Predict Cancer"):
        input_data = np.array([[GENDER, AGE, SMOKING, YELLOW_FINGERS, ANXIETY, PEER_PRESSURE, CHRONIC_DISEASE, FATIGUE, ALLERGY,  WHEEZING,    ALCOHOL_CONSUMPTION,  COUGHING, SHORTNESS_OF_BREATH, SWALLOWING_DIFFICULTY,  CHEST_PAIN]])
        prediction = cancer_model.predict(input_data)
        # st.write("Prediction (0 = Benign, 1 = Malignant): ", prediction[0])
        test_probabilities = cancer_model.predict_proba(input_data)[:, 1] 
        test_disease_levels = [classify_disease_level(prob) for prob in test_probabilities]
        st.write("Disease Level: ", test_disease_levels[0])
